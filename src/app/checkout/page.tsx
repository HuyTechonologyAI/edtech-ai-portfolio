...
import { Check, Copy, Sparkles, ShieldCheck, QrCode, RefreshCw, ArrowRight, CheckCircle2 } from "lucide-react";
...
const BANK_ID = "MB";
const ACCOUNT_NO = "0941214544";
const ACCOUNT_NAME = "NGO QUOC HUY";
...
export default function CheckoutPage() {
  ...
  // Generate unique order code on mount
  useEffect(() => {
    // Generate a clean 6-digit transaction memo format: EXPERT + 6 random digits
    const randomDigits = Math.floor(100000 + Math.random() * 900000);
    setOrderCode(`EXPERT${randomDigits}`);
  }, []);
  ...
  const qrUrl = `https://img.vietqr.io/image/${BANK_ID}-${ACCOUNT_NO}-compact2.png?amount=${selectedPlan.price}&addInfo=${orderCode}&accountName=${encodeURIComponent(ACCOUNT_NAME)}`;

  const handleVerifyPayment = () => {
    setIsVerifying(true);
    // Simulate real-time polling to webhook reconciliation backend
    setTimeout(() => {
      setIsVerifying(false);
      setIsSuccess(true);
      // Automatically reload session or trigger local premium state for premium user feedback
    }, 3000);
  };

  return (
    <main className="flex-1 py-12 bg-background relative overflow-hidden">
      ...
      <div className="container px-4 md:px-6 max-w-6xl mx-auto relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/10 border border-secondary/20 text-secondary text-xs font-bold uppercase tracking-wider mb-3">
            <Sparkles className="w-3.5 h-3.5" /> Nâng cấp tài khoản
          </div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-3">
            Đầu Tư Cho <span className="text-secondary neon-glow-text">Tri Thức &amp; Tự Động Hóa</span>
          </h1>
          <p className="text-foreground/70 text-sm md:text-base">
            Hệ thống thanh toán tự động qua VietQR. Tài khoản của bạn sẽ được kích hoạt Premium tự động ngay sau khi chuyển khoản thành công.
          </p>
        </div>
        ...
        {isSuccess ? (
          <div className="max-w-md mx-auto glass-panel rounded-3xl p-8 text-center border border-secondary/30 shadow-[0_0_50px_rgba(0,255,133,0.15)] animate-fade-in">
            ...
            <p className="text-foreground/70 text-sm mb-6">
              Cảm ơn bạn đã nâng cấp gói <span className="text-secondary font-bold">{selectedPlan.name}</span>. Quyền lợi Premium của bạn đã được kích hoạt trên hệ thống.
            </p>
            <div className="p-4 rounded-xl bg-background/50 border border-border mb-6 text-left text-xs space-y-2">
              ...
              <div className="flex justify-between"><span className="text-foreground/50">Thời hạn:</span> <span className="font-bold text-secondary">Trọn đời</span></div>
            </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="glass-panel rounded-3xl p-6 border border-secondary/30 shadow-[0_0_40px_rgba(0,255,133,0.08)] relative">
                ...
                <div className="bg-white p-4 rounded-2xl max-w-[260px] mx-auto mb-6 shadow-xl relative group">
                  {/* Aspect ratio container for stable QR load */}
                  ...
                  <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl flex items-center justify-center text-center p-3">
                    <p className="text-xs text-white font-medium">Sử dụng App ngân hàng bất kỳ để quét mã VietQR</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between items-center bg-background/60 p-2.5 rounded-xl border border-border">
                      ...
                      <span className="font-bold text-foreground">{BANK_ID} Bank</span>
                    </div>

                    <div className="flex justify-between items-center bg-background/60 p-2.5 rounded-xl border border-border">
                      <span className="text-foreground/50">Số tài khoản:</span>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-foreground">{ACCOUNT_NO}</span>
                        <button 
                          onClick={() => handleCopy(ACCOUNT_NO, "acc")}
                ...
                <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[11px] text-amber-400/90 leading-relaxed text-center">
                  ⚠️ <strong className="text-amber-400">Lưu ý quan trọng:</strong> Vui lòng nhập chính xác nội dung chuyển khoản là <strong className="text-secondary font-mono">{orderCode}</strong> để hệ thống tự động kích hoạt gói trong 5 giây.
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-border">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-secondary text-black font-bold text-sm hover:bg-secondary/90 hover:shadow-[0_0_25px_rgba(0,255,133,0.4)] transition-all disabled:opacity-50 disabled:pointer-events-none"
                  >
                    {isVerifying ? (
                      <>
                  ...
                  {!user && (
                    <p className="text-center text-[11px] text-foreground/40 mt-2.5">
                      💡 Bạn đang giao dịch ở chế độ Khách. Hệ thống sẽ cấp tài khoản tự động dựa trên email thanh toán của bạn.
                    </p>
                  )}
                </div>
              </div>
